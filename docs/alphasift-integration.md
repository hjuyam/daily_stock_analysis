...
- 已确认工作树无未解析 merge marker（`<<<<<<<`/`=======`/`>>>>>>>`），当前修订基于冲突已清理后的 head 重跑验证结果。
- 关键后端契约与安装状态链路：
  - `python -m pytest tests/test_alphasift_api.py -q`（13 passed）。
  - 相关配置面向回归：`python -m pytest tests/test_system_config_service.py tests/test_llm_channel_config.py tests/test_config_validate_structured.py tests/test_config_env_compat.py tests/test_task_queue_config_sync.py -q`（共 253 passed）。
- Web 侧兼容性回归：`cd apps/dsa-web && npm run lint`（通过）、`npm run test -- --run`（56 passed，496 tests，2 skipped）与 `npm run build`（成功）。
- 与 LLM 配置相关：
